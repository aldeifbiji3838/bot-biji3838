...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:√ALDEIF√\n' // Nama Lu
            + 'ORG:#OwnerGanz;\n' // Nama Kontak Owner
            + 'TEL;type=CELL;type=VOICE;waid=6285717833875:+62 857-1783-3875\n' // Nomer Lu cok
            + 'END:VCARD'
prefix = "#"
blocked = []
	...
	anker.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6285717833875@s.whatsapp.net'
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══\n╠➥User'+teks+'╚═〘 Biji BOT BOT 〙', members_id, true)
					break
                case 'tagall2':
				anker.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					anker.sendMessage(from, '╔══✪〘 Mention All 〙✪══\n╠➥User'+teks+'╚═〘 Biji BOT BOT 〙', text, {quoted: mek})
					break
                case 'tagall3':
				anker.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					anker.sendMessage(from, '╔══✪〘 Mention All 〙✪══\n╠➥User'+teks+'╚═〘 Biji BOT BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				anker.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
